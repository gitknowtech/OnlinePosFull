...
export default function UnitModel({ UserName, store }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/create_units', {
        unitName: unitName,
        user: UserName,
        store: storeValue,
        saveTime,
  ...
  const handleDelete = async (unitName) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete('http://localhost:5000/api/delete_unit', {
    ...
    <div className="unit-model">
      ...
      <div className="unit-grid">
        ...
        <div className="pagination">
          {Array.from({ length: Math.ceil(filteredUnits.length / itemsPerPage) }).map(
            (_, index) => (
              <button
                key={index}
